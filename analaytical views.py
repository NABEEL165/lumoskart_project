...
def admin_dashboard(request):
    ...
    daily_labels = [item['day'].strftime('%b %d') for item in daily_revenue_data]
    daily_revenue_values = [float(item['revenue'] or 0) for item in daily_revenue_data]
    daily_orders = [item['orders'] for item in daily_revenue_data]
    
    # Yearly revenue
    yearly_data = Order.objects.filter(status='Completed').annotate(
    ...
    yearly_labels = [item['year'].strftime('%Y') for item in yearly_data]
    yearly_revenue_values = [float(item['revenue'] or 0) for item in yearly_data]
    yearly_orders = [item['orders'] for item in yearly_data]
    ...
    for influencer_data in influencer_earnings.values():
        influencer_performance.append({
            'username': influencer_data['user'].username,
            'total_earnings': influencer_data['total_earnings'],
            'orders_count': OrderItem.objects.filter(
                product__influencer=influencer_data['user'],
                order__status='Completed'
            ).count()
        })
    influencer_performance = sorted(influencer_performance, key=lambda x: x['total_earnings'], reverse=True)[:10]
    
    # Customer cohorts (by registration month)
    customer_cohorts = CustomUser.objects.filter(user_type='customer').annotate(
        cohort_month=TruncMonth('date_joined')
    ).values('cohort_month').annotate(
        customer_count=Count('id')
    ).order_by('cohort_month')
    
    cohort_labels = [item['cohort_month'].strftime('%b %Y') for item in customer_cohorts]
    cohort_values = [item['customer_count'] for item in customer_cohorts]
    
    # Traffic sources (mock data - would come from analytics service)
    raw_traffic_sources = [
        {'source': 'Google', 'visits': 15420, 'conversions': 324},
        {'source': 'Instagram', 'visits': 8930, 'conversions': 187},
        {'source': 'YouTube', 'visits': 6210, 'conversions': 98},
        {'source': 'Direct', 'visits': 4320, 'conversions': 76},
    ]
    
    # Calculate conversion rates for each source
    traffic_sources = []
    for source in raw_traffic_sources:
        conversion_rate = (source['conversions'] / source['visits'] * 100) if source['visits'] > 0 else 0
    ...
    total_conversions = sum(source['conversions'] for source in traffic_sources)
    overall_conversion_rate = (total_conversions / total_visits * 100) if total_visits > 0 else 0
    
    # Search terms (would come from search analytics)
    search_terms = [
        {'term': 'smartphone', 'searches': 1247, 'conversions': 34},
        {'term': 'laptop', 'searches': 892, 'conversions': 28},
        {'term': 'headphones', 'searches': 654, 'conversions': 19},
        {'term': 'camera', 'searches': 523, 'conversions': 15},
        {'term': 'watch', 'searches': 487, 'conversions': 12},
    ]
    ...
    context = {
        ...
        'daily_revenue_values': json.dumps(daily_revenue_values),
        'daily_orders': json.dumps(daily_orders),
        'yearly_labels': json.dumps(yearly_labels),
        'yearly_revenue_values': json.dumps(yearly_revenue_values),
        'yearly_orders': json.dumps(yearly_orders),
        'product_performance': product_performance,
        'influencer_performance': influencer_performance,
        'cohort_labels': json.dumps(cohort_labels),
        'cohort_values': json.dumps(cohort_values),
...
def manage_thumbnails(request):
    ...
    # Handle each query individually to prevent failures if any model is unavailable
    products = []
    blog_posts = []
    videos = []
    promo_videos = []
    ...
    try:
        ...
        pass  # Product model may not be available
    
    try:
        blog_posts = BlogPost.objects.all()
    except:
        pass  # BlogPost model may not be available
    
    try:
        videos = InfluencerVideo.objects.all()
    except:
        pass  # InfluencerVideo model may not be available
    
    try:
        promo_videos = PromoVideo.objects.all()
    except:
        pass  # PromoVideo model may not be available

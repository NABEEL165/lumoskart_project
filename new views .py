...
def influencer_dashboard(request):
    ...
    if float(previous_month_revenue) > 0:
        total_revenue_change = min(100, int((float(total_revenue) / float(previous_month_revenue)) * 100))
    ...
    if float(previous_month_orders) > 0:
        total_orders_change = min(100, int((float(total_orders) / float(previous_month_orders)) * 100))

    monthly_revenue_change = 0
    if float(previous_month_revenue) > 0:
        monthly_revenue_change = min(100, int((float(monthly_revenue) / float(previous_month_revenue)) * 100))
...
@login_required
def influencer_pending_orders(request):
    if request.user.user_type != 'influencer':
        return redirect('home')

    # Get pending orders for this influencer (confirmed but not shipped)
    pending_orders = Order.objects.filter(
        items__product__influencer=request.user,
    ...
    context = {
        ...
        'order_type': 'Pending',
        'status_description': 'Orders confirmed but not yet shipped'
    }

    return render(request, 'influencer_orders.html', context)


@login_required
def influencer_shipped_orders(request):
    if request.user.user_type != 'influencer':
        return redirect('home')

    # Get shipped orders for this influencer (dispatched and in transit)
    ...
    context = {
        ...
        'order_type': 'Shipped',
        'status_description': 'Orders dispatched and in transit'
    }
...
def admin_dashboard(request):
    ...
    if total_commission == 0:
        ...
        total_commission = 0  # Initialize to 0
        for order in completed_orders:
            if order.total_amount:
                try:
                    commission_percentage = order.commission_percentage
                    total_commission += float(order.total_amount) * (float(commission_percentage) / 100)
                except AttributeError:
                    # If commission_percentage field doesn't exist, use a default of 10%
                    total_commission += float(order.total_amount) * (20.0 / 100)

    commission_percentage = round((float(total_commission) / float(total_revenue) * 100), 2) if total_revenue and total_revenue != 0 else 0
    total_influencer_earnings = float(total_revenue) - float(total_commission)
    ...
    for order in orders:
        for item in order.items.all():
            ...
            try:
                ...
                commission_percentage = 20.0  # Default 10% commission
            earnings = float(item.price) * float(item.quantity) * (1 - float(commission_percentage) / 100)
...
@login_required
def reset_influencer_password(request, user_id):
    if not request.user.is_staff:
        return redirect('home')
    ...
    if request.method == 'POST':
        # Check if a custom password was provided
        ...
        influencer.set_password(temp_password)
        influencer.save()

        # Send email notification with the temporary password
        try:
            send_mail(
                'Password Reset - LumosKart',
            ...
            messages.success(request, f'Password has been reset successfully for {influencer.username}. A password has been sent to their email.')
        except Exception as e:
            # If email fails, still notify the admin
            messages.warning(request, f'Password reset for {influencer.username}, but email notification failed. Please inform the user manually.')
...
def influencer_earnings(request):
    ...
    for item in influencer_orders:
        try:
            commission_percentage = item.order.commission_percentage
            total_earnings += float(item.price) * item.quantity * (1 - float(commission_percentage) / 100)
        except AttributeError:
            # If commission_percentage field doesn't exist, use a default of 20%
            total_earnings += float(item.price) * item.quantity * (1 - 20.0 / 100)
    ...
    pending_withdrawals_decimal = WithdrawRequest.objects.filter(
        influencer=request.user,
        status='pending'
    ).aggregate(total=Sum('amount'))['total'] or 0
    pending_withdrawals = float(pending_withdrawals_decimal)
...
def accept_pending_order(request, order_id):
    ...
    # Check if the order contains products from this influencer
    order_items = order.items.all()
    influencer_products = [item for item in order_items if item.product.influencer == request.user]

    if not influencer_products:
        messages.error(request, 'You are not authorized to accept this order.')
        return redirect('influencer_pending_orders')

    # If the order is still pending, update its status to Shipped
    if order.status == Order.PENDING:
        # When an influencer accepts a pending order, they're taking responsibility for it
        # So we'll move it to SHIPPED status since they'll be handling the shipment
        order.status = Order.SHIPPED
        order.save()
        messages.success(request, f'Order #{order.id} has been accepted and moved to shipped status.')
    else:
        messages.error(request, 'This order cannot be accepted at this time.')
...
@login_required
def complete_shipped_order(request, order_id):
    if request.user.user_type != 'influencer':
        return redirect('home')
    ...
    if not influencer_products:
        messages.error(request, 'You are not authorized to complete this order.')
        return redirect('influencer_shipped_orders')

    # If the order is shipped, update its status to Completed
    if order.status == Order.SHIPPED:
        # When an influencer marks a shipped order as complete,
        # it means the customer has received it
        order.status = Order.COMPLETED
        order.save()
        messages.success(request, f'Order #{order.id} has been marked as completed.')
    else:
        messages.error(request, 'This order cannot be marked as completed at this time.')
...
def mark_order_delivered(request, order_id):
    ...
    if order.status in [Order.SHIPPED, Order.PENDING]:
...
def download_invoice(request, order_id):
    ...
    context = {
        'order': order,
        'order_items': order_items,
        'company_name': 'LumosKart',
        'company_address': '123 Business Street, City, State 12345',
        'company_email': 'contact@lumoskart.com',
...
@login_required
def export_admin_dashboard_data(request):
    if not request.user.is_staff:
        return redirect('home')
    ...
    # Get dashboard data similar to admin_dashboard view
    today = timezone.now()
    ...
    top_influencers = (
        CustomUser.objects
        .filter(user_type='influencer', is_active=True)
        .annotate(
            total_revenue=Sum(
                F('product__orderitem__order__total_amount'),
                filter=Q(product__orderitem__order__status='Completed')
            )
        )
    ...
    return render(request, 'admin_dashboard_export.html', context)




@login_required
def export_manage_orders_data(request):
    if not request.user.is_staff:
        return redirect('home')

    # Get all orders with related data
    orders = Order.objects.select_related('user').prefetch_related('items__product__influencer').order_by('-created_at')

    # Apply the same filters that are used in manage_orders view
    influencer_filter = request.GET.get('influencer')
    customer_filter = request.GET.get('customer')
    status_filter = request.GET.get('status')
    date_filter = request.GET.get('date')
    ...
    # Remove duplicates if filtering caused joins
    orders = orders.distinct()
    ...
    # Status breakdown
    status_counts = {}
    for status_choice, _ in Order.ORDER_STATUS_CHOICES:
        status_counts[status_choice] = orders.filter(status=status_choice).count()
    ...
    return render(request, 'manage_orders_export.html', context)
